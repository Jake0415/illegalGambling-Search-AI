...
### Task 049: 구현 — 검색 기반 탐지 및 다층 키워드 관리

| 항목 | 내용 |
|------|------|
| **예상 소요** | 7일 |
| **선행 Task** | Task 048 (Scrapy 크롤링 인프라) |
| **PRD 참조** | FR-DE-001~004, FR-DE-036~050 |
...
- [ ] 4. **탐지 결과 -> 사이트 등록 파이프라인**: 중복 제거, AI 분류, 채증 큐 등록

**구현 사항 (다층 키워드 체계 보강):**
- [ ] 5. **다층 키워드 분류**: KeywordLayer enum (DIRECT/BAIT/VERIFICATION/COMMUNITY/SEASONAL), BaitSubtype enum
- [ ] 6. **미끼 키워드 시드 데이터 30개+**: 무료 스포츠 중계, 무료 드라마, 먹튀검증, 꽁머니 등
- [ ] 7. **한국어 변형 매핑 엔진**: 초성(ㅌㅌ), 오타(토1토), 한영혼합(카sino), 숫자치환, 형태소분해
- [ ] 8. **keyword_variants 테이블**: 원본 1개당 5~15개 변형 자동 생성·저장
- [ ] 9. **keywords 테이블 확장**: layer, bait_subtype, precision, TP/FP, api_call_count, source 필드

---

### Task 049-B: 구현 — 키워드 자동 발견 및 효과성 추적

| 항목 | 내용 |
|------|------|
| **예상 소요** | 7일 |
| **선행 Task** | Task 049 |
| **PRD 참조** | FR-DE-037~047 |

**구현 사항:**
- [ ] 1. **메타태그 키워드 추출**: 크롤링 사이트 HTML에서 meta/title/h1~h3/OG 태그 키워드 추출, Googlebot UA 재요청
- [ ] 2. **Claude AI 키워드 확장**: 매주 1회, 4가지 전략(동의어/변형/미끼발견/트렌드), 프롬프트 캐싱
- [ ] 3. **Naver/Google 자동완성 수집**: 시드 20개, Claude 필터링, 매일 1회
- [ ] 4. **키워드 후보 검토 큐**: keyword_candidates CRUD, 관리자 일괄 승인/반려, 승인→keywords 자동 등록
- [ ] 5. **미끼 키워드↔사이트 연결 추적**: keyword_site_links (SEARCH_RESULT/REDIRECT/POPUP/AD_CLICK/META_TAG)
- [ ] 6. **정밀도 자동 계산**: AI 분류 승인/반려 → TP/FP 집계, 30%↓ 비활성화 후보, 70%↑ 고효율 태그
- [ ] 7. **비용 효율 분석**: cost_per_detection, 비용 기반 검색 빈도 자동 조정
- [ ] 8. **시즌/이벤트 키워드**: sports_events 캘린더, 7일 전 자동 활성화, 스캔 2배
- [ ] 9. **Kiwi 형태소 분석 통합**: 연속문자열 분리, 조사 제거, 띄어쓰기 정상화
...
| **예상 소요** | 5일 |
| **선행 Task** | Task 048 |
| **PRD 참조** | FR-DE-015~019, FR-DE-031~035 |
...
- [ ] 5. **SSL 인증서 SAN 분석**: TLS 접속 시 SAN 필드 추출 → 관련 도메인 자동 발견, 인증서 fingerprint 기반 클러스터링
- [ ] 6. **CT 로그 모니터링**: crt.sh API로 신규 인증서 발급 감시, 클러스터 패턴 기반 선제 탐지
- [ ] 7. **JARM TLS 핑거프린팅**: 10개 TLS 프로브 → 62자 해시 생성, 도메인 변경 후 동일 서버 재사용 탐지
- [ ] 8. **GA/GTM/Pixel ID 역추적**: HTML에서 트래킹 코드 정규식 추출, 동일 ID 공유 도메인 자동 클러스터링
- [ ] 9. **Passive DNS 연동** (Phase 3): SecurityTrails/VirusTotal API로 과거 DNS 이력 조회, IP 기반 역추적

---

### Task 050-B: 구현 — 판단 시나리오 보강 (LLM/규칙 앙상블)

| 항목 | 내용 |
|------|------|
| **예상 소요** | 5일 |
| **선행 Task** | Task 048, Task 049 |
| **PRD 참조** | FR-DE-051~055 |

**구현 사항:**

- [ ] 1. **LLM↔규칙 충돌 해소** (P0): 신뢰도 가중 투표 시스템(w_rule=0.3, w_llm=0.7), 충돌 유형별 처리 전략 4종, 충돌 검토 큐, 가중치 자동 최적화(주 1회)
- [ ] 2. **실시간 페이지 유형 판단**: 3단계 파이프라인(규칙 즉시 분류 → Haiku 텍스트 → Vision 스크린샷), 페이지 전이 FSM(MAIN→REGISTER→BETTING→DEPOSIT)
- [ ] 3. **JS 동적 콘텐츠 판단**: WebSocket 메시지 감청(배당률/배팅 데이터), 렌더링 전/후 스크린샷 비교, SPA 내부 라우팅 추적, DOM MutationObserver
- [ ] 4. **위장/클로킹 사이트 탐지**: 이중 UA 요청(일반/Googlebot) HTML diff 비교, IIS 악성코드 시그니처 DB, 관문사이트(외부 링크 도박 비율 >50%) 판정
- [ ] 5. **다국어 콘텐츠 판단**: `franc` 언어 감지, 한/영/중 도박 용어 사전 각 50개+, Claude Haiku 다국어 few-shot 프롬프트

---

### Task 051: 구현 — 크론 스케줄 + 채증 스케줄 관리 + 통계 대시보드

| 항목 | 내용 |
|------|------|
| **예상 소요** | 5일 |
| **선행 Task** | Task 034 (Celery), Task 049, Task 050 |
| **PRD 참조** | FR-DE-024, FR-EC-035~038, FR-UI-036~038, FR-API-046~053 |

**구현 사항 (탐지 스케줄):**
- [ ] 1. **Celery Beat 기반 크론 스케줄**: 키워드 탐지 (매일), 도메인 체크 (30분), 사이트 재검증 (주 1회)
- [ ] 2. **Slack 자동 보고**: 스케줄 실행 결과 Slack 전송

**구현 사항 (채증 스케줄 관리):**
- [ ] 3. **investigation_schedules + schedule_run_logs 테이블**: Alembic 마이그레이션
- [ ] 4. **스케줄 CRUD API**: GET/POST/PATCH/DELETE /api/investigation-schedules, 실행 이력 조회
- [ ] 5. **스케줄 관리 UI** (`/investigations/schedules`): 카드 목록, 생성 Dialog (6가지 반복 유형, 대상 필터, 시간대 설정), 활성화/비활성화, 실행 이력
- [ ] 6. **상시 모니터링 모드**: CONTINUOUS 스케줄, 큐 drain 이벤트 감지, 우선순위 자동 투입

**구현 사항 (채증 통계 대시보드):**
- [ ] 7. **채증 통계 API**: GET /api/investigations/stats (KPI + 일별 건수 + 시간대 분포 + 추이)
- [ ] 8. **일별 상세 API**: GET /api/investigations/stats/daily/:date (타임라인 + 스크린샷)
- [ ] 9. **채증 통계 페이지** (`/investigations/stats`): KPI 4개, 캘린더 히트맵, 시간대 바차트, 일별 타임라인, 스크린샷 라이트박스

**구현 사항 (일괄 채증):**
- [ ] 10. **일괄 채증 API**: POST /api/investigations/batch (siteIds 최대 50건, batch_id)
- [ ] 11. **사이트 목록 체크박스**: `/sites` 테이블에 다중 선택 + "일괄 채증 시작" 액션 바
- [ ] 12. **채증 시작 Dialog 확장**: 다중 사이트 추가/제거, batch_id 기반 진행 조회
