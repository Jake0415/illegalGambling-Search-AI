...
본 섹션은 불법 도박 사이트 자동 검색/채증 시스템의 REST API 명세를 정의한다. 모든 API는 FastAPI (Python) 라우터로 구현하며, Pydantic v2 모델을 통해 요청/응답 유효성 검증과 자동 OpenAPI 문서 생성을 수행한다.

### 핵심 기술 스택

| 영역 | 기술 | 비고 |
|------|------|------|
| API 프레임워크 | FastAPI + Uvicorn | Python 비동기 웹 프레임워크, 자동 OpenAPI 문서 생성 |
| 인증/인가 | FastAPI Security (python-jose + passlib) | JWT Bearer Token, RBAC |
| ORM | SQLAlchemy 2.x + Alembic + PostgreSQL | 비동기 세션 지원, 마이그레이션 관리 |
| 유효성 검증 | Pydantic v2 | 요청 본문/쿼리 파라미터 검증, 자동 직렬화 |
| 비동기 작업 큐 | Celery + Redis | 채증/탐지 비동기 작업 관리 |
| 파일 저장소 | MinIO (boto3 SDK) | 스크린샷, 증거 파일, 보고서 |
...
FastAPI Security 모듈 기반 JWT Bearer Token 인증을 사용한다 (python-jose로 토큰 생성/검증, passlib로 비밀번호 해싱).
...
| 400 | `VALIDATION_ERROR` | Pydantic v2 스키마 검증 실패 |
...
| ID | 제목 | 설명 | 우선순위 | Phase | 수용 기준 | 기술 구현 참고 |
|----|------|------|---------|-------|----------|---------------|
| **FR-API-031** | GET /api/manual-queue -- 수동 개입 큐 목록 조회 | CAPTCHA 자동 풀이 실패 또는 SMS OTP 자동 입력 실패로 수동 개입이 필요한 세션 목록을 조회한다. 각 세션의 개입 유형(`captcha`, `otp`, `unknown_form`), 대기 시간, 사이트 URL, 현재 스크린샷 미리보기, 만료까지 남은 시간을 포함한다. 대기 시간 기준 내림차순(오래된 것 우선)으로 정렬한다. | P1 | Phase 2 | 1. 요청 시 수동 개입 대기 중인 세션 목록이 대기 시간순으로 반환된다<br>2. 각 항목에 `id`, `type`(captcha/otp/unknown_form), `siteUrl`, `waitingSince`, `timeRemaining`, `screenshotUrl`, `investigationId` 필드가 포함된다<br>3. 타임아웃(5분) 초과 세션은 자동으로 `expired` 상태로 전환된다<br>4. `type` 필터로 개입 유형별 조회가 가능하다<br>5. 실시간 업데이트가 SSE를 통해 제공된다 | Celery (수동 개입 큐), Redis (세션 상태 TTL), MinIO presigned URL (스크린샷), SSE (실시간 업데이트) |
| **FR-API-032** | POST /api/manual-queue/:id/resolve -- 수동 개입 완료 처리 | 수동 개입 세션의 완료를 처리한다. CAPTCHA 풀이 완료 또는 OTP 입력 완료 후 자동화 파이프라인을 재개한다. 개입자 ID, 소요 시간, 결과(success/failed)를 기록한다. 실패 시 해당 세션을 `failed`로 처리하고 채증 작업을 다음 단계로 진행하거나 스킵한다. | P1 | Phase 2 | 1. 완료 처리 시 브라우저 자동화가 재개되고 HTTP 200을 반환한다<br>2. `result: "success"` 시 채증 파이프라인이 다음 단계로 진행된다<br>3. `result: "failed"` 시 해당 단계를 스킵하고 가능한 다음 단계로 진행된다<br>4. 만료된 세션에 대한 완료 요청 시 HTTP 410(`GONE`)을 반환한다<br>5. 개입 결과가 감사 로그에 개입자 ID, 소요 시간과 함께 기록된다 | Celery 태스크 완료 처리, rebrowser-playwright 세션 재개, SQLAlchemy 수동 개입 이력 |
...
| ID | 제목 | 설명 | 우선순위 | Phase | 수용 기준 | 기술 구현 참고 |
|----|------|------|---------|-------|----------|---------------|
| **FR-API-042** | GET /api/system/health -- 시스템 헬스 체크 | 시스템의 전반적인 건강 상태를 확인한다. 애플리케이션 서버, PostgreSQL 데이터베이스, Redis, Celery 큐, MinIO 스토리지의 연결 상태를 개별적으로 확인하고, 전체 상태(`healthy`, `degraded`, `unhealthy`)를 판별한다. 인증 없이 접근 가능하며, 로드 밸런서 및 모니터링 시스템의 헬스 체크 엔드포인트로 사용한다. | P0 | Phase 1 | 1. 인증 없이 접근 가능하다<br>2. 모든 서비스 정상 시 HTTP 200과 `status: "healthy"`를 반환한다<br>3. 일부 서비스 장애 시 HTTP 200과 `status: "degraded"` 및 장애 서비스 목록을 반환한다<br>4. 핵심 서비스(DB, Redis) 장애 시 HTTP 503과 `status: "unhealthy"`를 반환한다<br>5. 응답 시간이 500ms 이내이며, 개별 서비스 체크 타임아웃은 3초이다 | SQLAlchemy `text('SELECT 1')`, Redis `PING`, Celery Inspect 연결 확인, MinIO `head_bucket` (boto3), `time.monotonic()` 가동 시간 |
...
```
Phase 1 (MVP):
  FastAPI Security (python-jose + passlib)  -->  FR-API-001~004 (인증/RBAC)
  SQLAlchemy 2.x + PostgreSQL  -->  FR-API-005~011 (사이트 CRUD)
  Celery + Redis  -->  FR-API-012~014, 019 (채증 큐)
  MinIO (boto3)  -->  FR-API-020 (증거 조회)
  Python hashlib  -->  FR-API-022 (해시 검증)

Phase 2 (자동 채증):
  FR-API-001~004 (인증)  -->  모든 보호 API
  FR-API-012 (채증 생성)  -->  FR-API-015~018 (채증 관리)
  FR-API-020 (증거)  -->  FR-API-021~024 (증거 관리)
  Celery  -->  FR-API-025, 031~033 (탐지/수동 개입)
