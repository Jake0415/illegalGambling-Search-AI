...
export type KeywordSiteLinkType =
  | 'SEARCH_RESULT'
  | 'REDIRECT'
  | 'POPUP'
  | 'AD_CLICK'
...
export interface Keyword extends MutableEntity {
  ...
  effectivenessTag: EffectivenessTag | null;
...
/** 키워드 한국어 변형 (FR-DE-042) */
export interface KeywordVariant extends BaseEntity {
  keywordId: string;
  variantText: string;
...
/** 키워드 후보 (FR-DE-047) */
export interface KeywordCandidate extends BaseEntity {
  keyword: string;
  suggestedLayer: KeywordLayer;
  suggestedCategory: string | null;
  source: string;
  sourceDetail: Record<string, unknown> | null;
  status: CandidateStatus;
  similarKeywordIds: string[];
  reviewedById: string | null;
  reviewedAt: Date | null;
  approvedKeywordId: string | null;
}

/** 키워드-사이트 연결 (FR-DE-037) */
export interface KeywordSiteLink extends BaseEntity {
  keywordId: string;
  siteId: string;
  linkType: KeywordSiteLinkType;
  searchRank: number | null;
  discoveredAt: Date;
}

/** 키워드 주간 통계 (FR-DE-046) */
export interface KeywordWeeklyStats {
  id: string;
  keywordId: string;
  weekStart: string;
  detectionCount: number;
...
/** 스포츠 이벤트 (FR-DE-038) */
export interface SportsEvent extends MutableEntity {
  name: string;
  sportType: string;
  startDate: string;
  endDate: string;
